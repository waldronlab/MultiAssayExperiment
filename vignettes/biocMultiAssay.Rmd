...
# MultiAssay shortcut function for creating a MultiAssayExperiment

```{r MultiAssay}
ovlist <- lapply(ov, function(x) get(load(x)))
names(ovlist) <- tags
## use the "ov agilent" for masterpheno phenotype data:
lovhub2 <- MultiAssay(masterPheno=pData(ovlist[[2]]), Elist=ovlist) # drop=TRUE broken
lovhub2
format(object.size(lovhub2), units = "Mb")
```

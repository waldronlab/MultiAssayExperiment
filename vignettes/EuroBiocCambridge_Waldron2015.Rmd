...
- Interrogation of several molecule types in one set of biological samples
    + integration of ID-based and range-based data, with metadata
- Build on existing methods for storing data types
    + harmonize accessors, constructors, and methods of relevant Bioconductor classes
...
- Simplify complicated bookkeeping
    + manipulation of multiple assays simultaneously
...
* Provides a `show` method and generic `elist()` function

* Validity checks: 
    + `features()`, `samples()`, `subsetFeatures()`, `subsetSamples()` methods required
    + unique samples() for each element
...
* maintains complete data provenance
    + subsetting by feature, sample, and assay
    + tracks "keeps" and "drops" (just IDs)
...
Subsetting in one step by ranges
=================================================
...
Subsetting in one step by IDs
=================================================

```{r}
subsetted <- elist(subsetByFeature(myMultiAssay, "ENST00000355076"))
elist(subsetted)
```

`MultiAssayExperiment` is easy to create and use
=================================================


biocMultiAssay harmonizes Bioconductor vocabulary
=================================================
- Gene expression, methylation, mutations, microRNA, proteins, metabolites, etc.
...
features(matrix)
features(ExpressionSet)
features(RangedSummarizedExperiment)
features(GRanges)
...
- `biocMultiAssay` stores and integrates multiple different types of data in one object
    + currently in memory, will abstract on-disk and remote data
- Harmonizes subsetting and matching across all data types

...
3. `MultiAssayView`: Set up for subsetting operations
...
| pData | DataFrame  | Specimen-level phenotype data   |
...
* Validity checks: 
    + require `rownames()`, `colnames()`, `subsetFeatures()`, `subsetSamples()` methods
    + consistency checks on `colnames()` output when part of a `MultiAssayExperiment`
...
* Validity checks: 
    + items in `master` column must be found in rownames(pData)
    + items in `assay` column must be found in colnames(Elist[[i]])
    + `assayname` column must contain only and all `Elist` element names

`MultiAssayView` class and verb
=================================================
...
* `MultiAssayView` class structure:
...
Staging assays:
```{r}
MultiAssayView(myMultiAssay, "Affy", "assays")
```
...
\small
```{r}
MultiAssayView(myMultiAssay, c("Jack", "Jill"), "samples")
```
...
```{r}
MultiAssayView(myMultiAssay, "ENST00000355076", "features")
```
`MultiAssayView` class and verb
=================================================
...
MultiAssayView(myMultiAssay, rangeSubset, "features",
...
* uses `MultiAssayView` information to maintain complete data provenance
...
__Note:__ also provides **`colnames()`**, **"`[`"** (bracket), **`subsetSample()`**, and **`subsetFeature()`** functions to harmonize classes

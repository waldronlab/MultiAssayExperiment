...
vector `data` is created to specify the location of the data directory where
output should be saved.

```{r}
ds <- getFirehoseDatasets()[27] # PRAD
...
(e.g. `ds <- getFirehoseDatasets()[1:5]`). 
...
buildMultiAssayExperiments <-
    function(TCGAcodes, runDate, analyzeDate) {
        for (cancer in TCGAcodes) {
            ...
            if (file.exists(serialpath)) {
                ...
                cancer.object <- getFirehoseData(cancer, runDate = runDate,
            ...
            for (i in seq_along(clinical.data)){
              clinical.data[[i]] <- type.convert(clinical.data[[i]])
            }
            ...
            newmap <- generateMap(data.full, clinical.data, TCGAbarcode)
...
```{r}
mae <- buildMultiAssayExperiments(TCGAcodes=ds, runDate=runDate,

...
# RTCGAToolbox
if (!is.installed("RTCGAToolbox")){
  BiocInstaller::biocLite("LiNk-NY/RTCGAToolbox")
}
library(RTCGAToolbox)

# MultiAssayExperiment
if (!is.installed("MultiAssayExperiment")){
  ...
  for(i in ds) {
    ...
    for(i in slotNames(co)[5:17]) {
      ...
      tryCatch({
        ...
        if(push2el == TRUE){
          nl <- c(nl, i)
          if(inherits(get(i), "GRangesList")) {
            RangedRaggedAssay(get(i))
          }
          el <- c(el, get(i))
        }
      })
    ...
    names(el) <- nl
    cel <- cleanExpList(el, pd)
    map <- generateTCGAmap(el, pd)

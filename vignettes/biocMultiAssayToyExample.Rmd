...
class(masterPheno(empty)) #DataFrame
class(sampleMap(empty))   #DataFrame
class(metadata(empty))    #NULL (class "ANY")
methods(class="MultiAssayExperiment")
methods(class="RaggedRangedAssay")
getMethod("colnames", "RaggedRangedAssay")
```
...
```{r}
MultiAssayView_cl <- Stage(myMultiAssay, 2:1, "colnames")
MultiAssayView_cl
subMultiAssay <- subset(myMultiAssay, MultiAssayView_cl, "sample")
as.list(Elist(subMultiAssay))
subsetByAssay(subMultiAssay, c(TRUE, FALSE, FALSE, TRUE, FALSE), drop = TRUE)[[1]] %>% exprs 
subMultiAssay
```
...
### TODO: Fix drop argument in subset

```{r, eval = FALSE}
subset(myMultiAssay, "Mirna", "assays", drop = TRUE)
```
...
```{r}
Stage(myMultiAssay, "ENST00000355076", "rownames")  
```
...
```{r}
featSubsetted0 <- subset(myMultiAssay, Stage(myMultiAssay, "ENST00000355076", "rownames"))
...
```{r}
featSubsetted <- subset(myMultiAssay, Stage(myMultiAssay, c("ENST00000355076", "ENST00000294241"), "rownames"))
exprs(Elist(myMultiAssay)[[1]])
exprs(Elist(featSubsetted)[[1]])
```

## Identify and stage assays that contain any of a vector of features

Note that the output of this function could be used as the input for
`subset`.

```{r}
Stage(myMultiAssay, c("ENST00000355076", "ENST00000294241"), "rownames") 
```

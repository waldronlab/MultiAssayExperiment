...
dfmap <- melt(listmap, id.var = c("master", "assay"))
names(dfmap)[3] <- "assayname"
...
# metadata(myMultiAssay)  #needs to be implemented
```
...
```{r, eval=FALSE}
logic_class <- identifyBySample(myMultiAssay, 2:1)
logic_class
subMultiAssay <- subsetBySample(myMultiAssay, logic_class)
...
(*TODO*: add checks on square bracket subsetting)

Here is what happens if one of the methods doesn't:
```{r}
objlist2 <- objlist
objlist2[[2]] <- data.frame(objlist2[[2]])
invalid.obj <- try(MultiAssayExperiment(objlist2, masPheno, idmap))
invalid.obj
```

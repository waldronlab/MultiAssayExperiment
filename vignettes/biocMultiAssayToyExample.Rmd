...
```{r}
source("../R/createMA.R")
source("../R/biocMultiAssay-classes.R")
MAE <- createMA(objlist, masterPheno, sampleMap = idmap)
```
...
chopper <- sapply(subMap, FUN = "[", 2)
browser()
subElist <- list()
for (i in 1:length(MAobject@elist)){
subElist[i] <- subsetSample(MAobject@elist[[i]], chopper[[i]])
}
return(subElist)
		   } else { 

...
class(sampleMap(empty))   #data.frame
...
```{r, message = FALSE}
dfmap <- biocMultiAssay:::.ldmap(listmap)
...
```{r, eval = FALSE}
stage_cl <- stage(myMultiAssay, 2:1, "samples")
stage_cl
subMultiAssay <- subsetBySample(myMultiAssay, id_class)

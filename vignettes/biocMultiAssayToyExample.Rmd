...
grl
# Specified row.names not preserved by mcols
mcols(grl) <- DataFrame(row.names = c("Jack", "Bob", "Barbara"),
	tss = rep("lung", 3), ptid = c("1221", "1321", "1322"))
```
...
Create an named list of experiments `objlist` for the createMA function
...
```{r}
MAE <- createMA(objlist, masPheno, idmap)
```

Subsetting by Sample

```{r}
ids <- identifyBySample(MAE, 1:2)

subsetBySample <- function(MAobject, idlist){
# Propose solution for rowIndex argument
	newPheno <- .subPheno(MAobject, rowIndex)
	newMap <- Map(subset, MAobject@sampleMap, idlist)
	newSubset <- Map(subsetSample, MAobject@elist, sapply(newMap, "[", 2))
	# Clone and replace slot? 
	MAobject@sampleMap <- newMap
	MAobject@elist <- S4Vectors::SimpleList(newSubset)
	MAobject@masterPheno <- newPheno
return(MAobject)
}

## -------
## TODO: Use Features to Subset MAE
## -------
## identifyByFeature <- function(MAobject, ){}

mysub <- subsetBySample(MAE, 1:2)
```

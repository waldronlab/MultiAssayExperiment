...
We start with a `GRangesList`, which can be converted using the
`RangedRaggedAssay` constructor function.
...
objlist <- list("Affy" = exprdat, "Methyl 450k" = methyldat,
                "Mirna" = microdat, "CNV gistic" = rra, "CNV gistic2" = rse)
```
...
```{r, eval = FALSE}
assay(myMultiAssay[[4]], background = 2)
```
...
The `assays()` function returns a `SimpleList` of data matrices from the `ExperimentList`:

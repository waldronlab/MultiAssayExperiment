...
from `PrepMultiAssay`. Named `Elist`s are essential for checks in
...
(`pData`), these will be dropped and added to the drops element. A similar
operation is performed for checking "primary" sampleMap names and `pData`
rownames.
...
Subsetting can be performed either using the `subset()` function, or even more
simply by square brackets.
...
equivalent ways (evaluate this chunk once working):
...
compact than the `subset()` method. The three positions within the bracket

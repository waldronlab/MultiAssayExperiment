...
The `complete.cases` function returns a logical vector identifying which
primary units have data for all experiments. Recall that `myMultiAssay`
...
The `duplicated` function returns a list of `LogicalList`s, identifying
replicate samples for each individual within each assay:

```{r}
duplicated(myMultiAssay)
```

# Functions to clean up a `MultiAssayExperiment`

A set of helper functions are available for clean-up operations. These include
`intersectRows`, `intersectPrimary`,`mergeReplicates`, `complete.cases`,
and `duplicated`.

The `intersectRows` function takes all common rownames across experiments and
returns a `MultiAssayExperiment` with those rows.

```{r}
(noMatches <- intersectRows(myMultiAssay))
```

`complete.cases` returns a logical vector of `colData` rows that correspond
to biological units that have data in all elements of the `ExperimentList`
...
A call to `intersectPrimary` returns another `MultiAssayExperiment` where the
columns of each element of the `ExperimentList` correspond exactly to the rows
of `colData`.

```{r}
intersectPrimary(myMultiAssay)
```

`duplicated` identifies `primary` column values or biological units that have
multiple observations per `assay`. It returns a `list` of `LogicalList`s
that indicate what biological units have one or more duplicated measurements.
This output is used for merging replicates by default.
...
combined with `intersectPrimary` to create a `MultiAssayExperiment` object with
one measurement in each experiment per biological unit. 

```{r}
mergeReplicates(intersectPrimary(myMultiAssay))
```
...
```{r}
rearrange(myMultiAssay[, , 1:2], colDataCols="age")
```
...
```{r}
maemerge <- mergeReplicates(intersectPrimary(myMultiAssay))
rearrange(maemerge, shape="wide", colDataCols="sex")[, 1:5]
```
...
the `API` wiki document on GitHub. From within an R session, type:

```{r, eval=FALSE}
API()
```

to produce the webpage for the API or use the "shiny" argument to get a `shiny`
app that browses the package, such as:
...
# Soon to be documented/developed

* `c()` function for adding new assays to existing `MultiAssayExperiment`
    + e.g. c(myMultiAssay, neweset)
    + require that sample names in the new object match colData sample names
    + require that sample names in the new object already exist in colData

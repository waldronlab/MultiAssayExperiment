...
`MultiAssayExperiment` class. It contains all the experiment data. It inherits
from class `S4Vectors::SimpleList` with one element/component per data type.

```{r}
class(experiments(empty)) # ExperimentList
```
The elements of the `ExperimentList` can contain **ID-based** and **range-based** data.
Requirements for all classes in the `ExperimentList` are listed in the API.
...
the 'biological unit' which can refer to specimens, experiment subject,
patients, etc. In this vignette, we will refer to each experiment subject as
a *patient*.
...
of the `ExperimentList`, where the experimental assays are stored, must be contained
in this column.
...
simpleMultiAssay <- MultiAssayExperiment(ExperimentList=doubleExp)
simpleMultiAssay
```
...
```{r}
simpleMultiAssay2 <- MultiAssayExperiment(ExperimentList=doubleExp, pData=patient.data)
simpleMultiAssay2
...
as those in the `MultiAssayExperiment` (i.e., `ExperimentList`, `pData`, `sampleMap`).
...
from `PrepMultiAssay`. Named `ExperimentList` elements are essential for checks in
`MultiAssayExperiment`.
...
When `colnames` in the `ExperimentList` cannot be matched back to the primary data
...
`RangedRaggedAssay` `ExperimentList` element) is that the metadata have a `score` column
...
```{r}
metadata(experiments(myMultiAssay)[[4]]) <- list(snparray1 = DataFrame(score = 1),
...
elements of `ExperimentList`, so when we refer to subsetting by columns, we are referring
...
DataFrame, then selects all columns of the `ExperimentList` corresponding to these rows.
...
This probably doesn't make sense unless every`ExperimentList`element represents the
same measurements in the same order and will generate an error if any of the
integer elements exceeds the number of rows in any`ExperimentList`element. The most
...
- a **list** or **CharacterList** of the same length as ExperimentList.  Each element
...
"drop=TRUE" is passed to subset, with any `ExperimentList` element not containing the

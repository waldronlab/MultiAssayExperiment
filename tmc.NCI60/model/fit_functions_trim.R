...
# summarize estimates of models prediction error 
# by repeated K-fold cross-validation (RCV)
# RCV is added with trimming of predictor matrix 
# after solution of linear dependencies in the predictor matrix
cross_validation <- function (K,R,X0,X1,Y){

 # K is folds number
 # R is repetitions number
 # X0 is the vector of observation of the predictor in M0
 # X1 is the matrix of observations of predictors in M1
 ...
 for(r in 1 : R){
  ...
  for(k in 1 : K){
   ...
   # remove predictors upon detection of linear dependencies
   if(!is.null(findLinearCombos(X1.train)$remove)){
   ...
   if(!is.null(warnings())){ 
    ...
    file="rcv.knn.trim.error.out",append=T,quote=F,row.names=F,col.names=F)
   }
   ...
   rmspe.se.M1 = NA
   rmspe = signif(as.numeric(rmspe(Y.predict,
    m1.predict$fit[,1],includeSE=T)[2]),digits=3)
   if(is.na(rmspe)==FALSE){ rmspe.se.M1=rmspe }
   ...
   # ratio of cor(predicted,observed) between M0 and M1
 ...
 if(is.null(m1.rmspe)==FALSE & is.null(m0.rmspe)==FALSE){ 
  ...
  # standard error of M1 RMSPE mean over K folds and R repetitions
  ...
  m0.rmspe.avg.se = signif((1/length(m0.rmspe.se))*sqrt(sum(m0.rmspe.se^2)),
   digits=3)
  # mean of RMSPE for M1 over K folds and R repetitions
